...
sys.path.append(os.path.join(BASE_DIR, "../../"))
from ops.teed_pointnet.pointnet2_batch import pointnet2_utils
...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, new_xyz=None) -> (torch.Tensor, torch.Tensor):
        ...
        if new_xyz is None:
            new_xyz = (
                pointnet2_utils.gather_operation(xyz_flipped, pointnet2_utils.farthest_point_sample(xyz, self.npoint)).transpose(1, 2).contiguous()
        ...
        *,
        ...
        for i in range(len(radii)):
            ...
            if self.dilated_group:
                ...
                self.groupers.append(
                    pointnet2_utils.QueryDilatedAndGroup(radius, min_radius, nsample, use_xyz=use_xyz) if npoint_list is not None else pointnet2_utils.GroupAll(use_xyz)
                )
            else:
                self.groupers.append(pointnet2_utils.QueryAndGroup(radius, nsample, use_xyz=use_xyz) if npoint_list is not None else pointnet2_utils.GroupAll(use_xyz))
            mlp_spec = mlps[i]
            if use_xyz:
                mlp_spec[0] += 3
        ...
        if (aggregation_mlp is not None) and (len(aggregation_mlp) != 0) and (len(self.mlps) > 0):
            ...
            for k in range(len(aggregation_mlp)):
                shared_mlp.extend([nn.Conv1d(out_channels, aggregation_mlp[k], kernel_size=1, bias=False), nn.BatchNorm1d(aggregation_mlp[k]), nn.ReLU()])
        ...
        if (confidence_mlp is not None) and (len(confidence_mlp) != 0):
            ...
            for k in range(len(confidence_mlp)):
                shared_mlp.extend([nn.Conv1d(out_channels, confidence_mlp[k], kernel_size=1, bias=False), nn.BatchNorm1d(confidence_mlp[k]), nn.ReLU()])
    ...
    def __init__(self, mlp_list, pre_channel, max_translate_range):
        ...
        if len(mlp_list) > 0:
            for i in range(len(mlp_list)):
                ...
                shared_mlps.extend([nn.Conv1d(pre_channel, mlp_list[i], kernel_size=1, bias=False), nn.BatchNorm1d(mlp_list[i]), nn.ReLU()])
